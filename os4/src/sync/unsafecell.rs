...
/// 将 UnSafeCell 标记为 Sync 使得它可以作为一个全局变量。
/// 这是 unsafe 行为，因为编译器无法确定我们的 UnSafeCell 能否安全的在多线程间共享。
/// 而我们能够向编译器做出保证，第一个原因是目前我们内核仅运行在单核上，因此无需在意任何多核引发的数据竞争/同步问题；
...
impl<T> UnSafeCell<T> {
    ...
    pub unsafe fn new(value: T) -> Self {
        // 希望使用者在创建一个 UnSafeCell 的时候，
        // 保证在访问 UnSafeCell 内包裹的数据的时候始终不违背上述模式：
